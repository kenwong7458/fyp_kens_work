...
function defaultRect() {
    return {type: "rectangle", x: 150, y: 150, width: 100, height: 100, fill: '#808080', name: genid()}
}
...
class Whiteboard extends React.Component {
    ...
    handleContextMenu = (e) => {
      ...
      this.setState({hiddenColorPicker: true}) //default: hidden color picker
      // clicked on stage/whiteboard - clear selection
      if (e.target === e.target.getStage() || e.target.name === "whiteboard") {
          this.setState({selectedShapeName: ''})
          return
      ...
      const name = e.target.name()
      console.log(name)
      this.setState({needUpdateObjName: name})
      const obj = this.state.objects.find(r => r.name === name)
      console.log(obj)
      // clicked on transformer - do nothing
      if (e.target.getParent().className === 'Transformer') return
      ...
      if(obj != null) {
        ...
        this.setState({hiddenColorPicker: false})
        console.log(this.state.hiddenColorPicker)
        var stage = e.currentTarget // same as: stage = this.stageRef.getStage(), or: stage = e.target.getStage()
        var x = obj.x+obj.width+'px'
        var y = obj.y+obj.height+'px'
        var colorPickerNewPosition = {x: x, y:y}
        this.setState({colorPickerPosition: colorPickerNewPosition})
        console.log(this.state.colorPickerPosition)


      }
    }

    handleRectColorChange = (color, event) => {
      const objName = this.state.needUpdateObjName
      ...
      if (!updateObj) {
        ...
        modifliedObj.fill = color.hex
        this.setState({objects: [...this.state.objects, modifliedObj]})
        this.setState({lineColor: color.hex})
      }
    }

    handleLineColorChange = (color, event) => {
      this.setState({lineColor: color.hex})

    }

    hiddenColorPicker = () => {
      this.setState({hiddenColorPicker: true})
    }
        ...
        return (
            <RndContainer id={id} {...other}>
                ...
                >
                            ...
                            <div className={classes.panel}>
                                ...
                                {this.state.canvasMode &&
                                    <Fragment>
                                        ...
                                        <div style={ colorPickerStyle.drawing} >
                                        <CompactPicker color={ this.state.lineColor}
                                                       onChangeComplete={ this.handleLineColorChange } />
                                        </div>
                                    </Fragment>
                                }
                            </div>
                    ...
                    { this.state.hiddenColorPicker ? null :
                      <div style={colorPickerStyle.popover}>
                        <CompactPicker color={ this.state.lineColor}
                                       onChangeComplete={this.handleRectColorChange}
                        />
                      </div>
                    }
                    <Stage width={800} height={600}
                        ...
                        onContextMenu={this.state.permission? this.handleContextMenu : this.handleNoPermission}
                        //onClick={this.state.permission? this.hiddenColorPicker : this.handleNoPermission}
                    >
                        {this.state.canvasMode &&
                        <Layer>
                            ...
                            {this.state.objects.map((obj, i) => {
                                ...
                                if (obj === undefined) console.log(obj)
                                    ...
                                    return <Rectangle
                                                ...
                                                onContextMenu={e => this.handleContextMenu}
                                            />
